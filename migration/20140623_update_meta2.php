...
try
{
	...
	foreach($ud_contents as $ud_content)
	{
		...
		foreach($bc_usr_meta_fields as $usr_meta_field)
		{
			...
			$upquery = "update BC_USR_META_FIELD set usr_meta_field_code='USR_$usr_meta_field_code' where ud_content_id=$ud_content_id and  usr_meta_field_id='$usr_meta_field[usr_meta_field_id]' ";
			$r = $db->exec($upquery);
		}
	}
	...
	foreach($bs_contents as $bs_content)
	{
		...
		foreach($bc_sys_meta_fields as $sys_meta_field)
		{
			...
			$upquery = "update BC_SYS_META_FIELD set sys_meta_field_code='SYS_$sys_meta_field_code' where bs_content_id=$bs_content_id  and sys_meta_field_id='$sys_meta_field[sys_meta_field_id]' ";
			$r = $db->exec($upquery);
		}
	}
