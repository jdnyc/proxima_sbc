...
//						$f().pause();
	...
	function updateReview(autoClose){
		...
		for (var i=0; i < records.length ; i++)
		{
			...
			if( records[i].data.review_status == <?=CONTENT_STATUS_REVIEW_READY?>) {
				Ext.Msg.alert('알림', '심의상태가 \"심의대기중\"이므로 심의할 수 없습니다.');
				return;
			}
				...
				if (success)
				{
					try
					{
						...
						else
						{
							...
							Ext.Ajax.request({
								url: '/php/review/send_mail_update.php',
								params: {
									params: Ext.encode(_r)
								},
								callback: function(opts, success, response){
...
//						}
				...
				listeners: {
					rowclick: function(self, rowIndex, e){
						try
						{
							...
							if ( !Ext.isEmpty(selected_row.get('reviewed_time')) )
							{
								Ext.getCmp('batch_reviewed_dt').setValue(new Date.parseDate(selected_row.get('reviewed_time'), 'YmdHis').format('Y-m-d H:i:s') );
							}
							if ( !Ext.isEmpty(selected_row.get('created_time')) )
							{
								Ext.getCmp('batch_requested_dt').setValue( new Date.parseDate(selected_row.get('created_time'), 'YmdHis').format('Y-m-d H:i:s') );
							}
			...
			items: [{
				...
				fieldLabel: '심의일시',
				id: 'batch_reviewed_dt',
				name: 'batch_reviewed',
				disabled: true
			},{
				id: 'batch_requested_dt',
				fieldLabel: '심의요청일',
				disabled: true
			}],
