		...
		buttons: [{
			...
			handler: function(b, e){
				...
				function doSubmit(own){
					var items = Ext.getCmp('user_metadata').getForm().items.items;
					...
					Ext.each(items, function(i){
						if (i.fieldLabel) 
						{
							...
							{
								case '방송일시':
									i.items.get(0).setValue(rPgmInfo.get('BDSTART_DATE'));
									i.items.get(1).setValue(rPgmInfo.get('BDSTART_TIME'));
								break;

								case '프로그램코드':
									i.setValue(rPgmInfo.get('PROG_CODE'));
								break;

								case '프로그램명':
									i.setValue(rPgmInfo.get('PROG_NAME'));
								break;

								case '모델':
									i.setValue(rPgmInfo.get('MODEL_NM'));
								break;

								case 'PD':
									i.setValue(rPgmInfo.get('PD_INFO'));
								break;

								case 'MD':
									i.setValue(rPgmInfo.get('MD_INFO'));
								break;

								case 'FD':
									i.setValue(rPgmInfo.get('FD_INFO'));
								break;

								case 'Video감독':
									i.setValue(rPgmInfo.get('VIDEO_INFO'));
								break;

								case '쇼호스트':
									i.setValue(rPgmInfo.get('SHOWHOST_INFO'));
								break;

								case '출연자':
									i.setValue(rPgmInfo.get('PLAY_NM'));
								break;

								case '오류유형':
									i.setValue(rPgmInfo.get('ERROR_TYPE'));
								break;

								case '오류내용':
									i.setValue(rPgmInfo.get('ERROR_DESCRIPTION'));
								break;
							}
						}
					});
