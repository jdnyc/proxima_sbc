...
//					field: Ext.getCmp('tranfer_item_combo').getValue(),
//					value: Ext.getCmp('fOnlyItemCD').getValue()
				}
			});
		},
		...
		buttons: [{
			...
			handler: function(b, e){
				...
				Ext.each(rItem, function(r, i, a){
					if (Ext.getCmp('transfer_ch_item_list').store.find('item_cd', r.get('item_cd')) < 0) 
					{
						Ext.getCmp('transfer_ch_item_list').store.add(r);
					}
				});
