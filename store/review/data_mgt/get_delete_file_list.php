...
*/
	...
	file_put_contents($logpath.'delete_'.basename(__FILE__).date('Ymd').'.log', '['.date("Y-m-d H:i:s").']query :'.$query."\n", FILE_APPEND);
	file_put_contents($logpath.'delete_'.basename(__FILE__).date('Ymd').'.log', '['.date("Y-m-d H:i:s").']data :'.print_r($delete_media_list, true)."\n", FILE_APPEND);
