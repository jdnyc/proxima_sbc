...
$edit_log = $db->queryAll("select * from bc_log where action = 'edit' and created_date between ".$s_date." and ".$e_date." order by created_date desc");
...
foreach($edit_log as $edit)
{
	...
	array_push($edit_rank['edit_rank'], array('rank'=>$i, 'type'=>$mappingMetaTable[$edit['content_id']], 'title'=>$content['title'], 'user'=>$user_name, 'date'=>$edit['created_date'], 'description'=>$edit['description']));
