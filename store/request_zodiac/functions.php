...
function matchingArticle($wsdl, $post,$test_user_id, $media_cd, $user_id, $channel){
    ...
    $ord_tr_id = $zodiac->addListTransmission($content_id, $user_id, $playout_id, $post['ord_id']);
	...
	if( $ord_tr_id ){
        ...
        $contentStatus = $contentService->findStatusMeta($content_id);
         ...
         if ( $contentStatus->scr_news_trnsmis_sttus == 'complete' || $contentStatus->scr_trnsmis_sttus == 'complete' || $contentStatus->scr_trnsmis_sttus == '3000' ) {
             //전송 완료
        ...
        //     $check_update = updateStatus($post['ord_id'], $user_id);
            ...
            $statusReturn = $zodiac->putUpdateContentsTransfer($param);
        ...
        }else if( !empty($contentStatus->scr_trnsmis_sttus) || !empty($contentStatus->scr_news_trnsmis_sttus) ){
            //진행중인경우
            if( $contentStatus->scr_trnsmis_sttus == 'request' || $contentStatus->scr_news_trnsmis_sttus == 'request'  ){
                $result =  array(
                    'success' => false,
                    'msg' => '전송 준비중입니다'
                );
...
function doTransmission($content_id, $user_id){
    ...
    $media_info_archive = $db->queryRow("
		SELECT	*
		FROM		BC_MEDIA
		WHERE	CONTENT_ID = ".$content_id."
    ...
    if( !empty($media_info_archive) ){
        $task = new TaskManager($db);
        $channel = 'transmission_zodiac_archive';
		$task_id = $task->start_task_workflow($content_id, $channel, $user_id);
