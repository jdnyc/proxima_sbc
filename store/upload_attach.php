...
try {
	...
	if (move_uploaded_file($tmp_filename, $new_file_name)) {
		...
		insertMedia($media_id, $content_id, 'Attach',$storage_info['storage_id'], $ud_content_id, $media_path, $_FILES['FileAttach']['size'], $ori_filename);
	} else {
...
function insertMedia($media_id, $content_id, $channel, $storage_id,$ud_content_id, $fullpath, $filesize, $memo = '')
{
	global $db;
	...
	if (!empty($memo)) {
		...
		$query = "insert into bc_media (media_id, content_id, media_type, storage_id, path, filesize, reg_type, created_date, expired_date, status)
                values ({$media_id}, {$content_id}, '{$channel}', {$storage_id}, '{$fullpath}', '$filesize', '{$channel}', '{$created_datetime}', '{$expired_date}', '0')";
	}
	$db->exec($query);
}
