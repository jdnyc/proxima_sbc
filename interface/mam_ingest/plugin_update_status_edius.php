...
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_updateStatus_edius'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] request '.$request."\n", FILE_APPEND);
...
//echo "insert_task_query_outside_data($content_id, $channel, $job_priority, $user_id, $filename)";exit;
	...
	@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_updateStatus_edius'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] return '.$return."\n", FILE_APPEND);
	...
	@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_updateStatus_edius'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] error '.$return."\n", FILE_APPEND);
	InterfaceClass::_LogFile('','return',$return);
	return $return;
}
