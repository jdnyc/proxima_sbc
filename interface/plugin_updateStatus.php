...
$flag = $_REQUEST['flag'];
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_updateStatus_'.$flag.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] request '.$request."\n", FILE_APPEND);
...
try{
	...
	if( empty($action) || empty($task_id) || empty($flag) ) {
		throw new Exception ('invalid request', 101 );
	}
...
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/aaa_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] 1 content_id 1 '.$content_id."\n", FILE_APPEND);
			
			//$content_id = $task_id;

			//skc 2019.01.25 TASK_ID를 보냈을때 오류가 나서 콘텐츠 ID 구하기 추가
			if(!empty($task_id)){
				$task_info = $db->queryRow("select * from bc_task where task_id=".$task_id);
				if(!empty($task_info)){
					$content_id = $task_info['src_content_id'];
				}
			}
			...
			$job_priority = 1;
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/aaa_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] 1 start '."\n", FILE_APPEND);
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/aaa_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] 1 content_id 2 '.$content_id."\n", FILE_APPEND);
			$channel = $flag;//'edius';
...
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/aaa_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] 1 end '."\n", FILE_APPEND);
	...
	@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_updateStatus_'.$flag.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] return '.$return."\n", FILE_APPEND);
	...
	@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_updateStatus_'.$flag.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] error '.$return."\n", FILE_APPEND);
	InterfaceClass::_LogFile('','return',$return);
	return $return;
}
