...
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_register_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] workflowInfo ===> '.print_r($workflowInfo, true)."\r\n", FILE_APPEND);

		if(in_array($workflowInfo['job_code'], array(ARIEL_TRANSFER_FS, ARIEL_TRANSFER_FTP, ARIEL_CATALOG, ARIEL_THUMBNAIL_CREATOR))) {
...
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/fcp_test_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] channel2222 ===> '.$channel."\r\n", FILE_APPEND);
			if($channel == 'fcpx'){
...
function insertContent($metaValues, $content_id, $category_id, $bs_content_id,
    ...
    global $db;
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_register_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] insertContent start' . '\n', FILE_APPEND);
    ...
    $v_return = $db->insert('BC_CONTENT', $insert_data);
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_register_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] BC_CONTENT ::: ' . $v_return . '\n', FILE_APPEND);
    $action = 'regist';
...
function insertMediaMetadata($content_id, $type, $filename, $channel) {
	...
	@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_register_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] insertMediaMetadata content_id ===> '.$content_id."\r\n", FILE_APPEND);
...
//				"($content_id, $media_id, $storage_id, '$type', '$path', $filesize, '$cur_time', '$register', '0',null,null)");
		...
		@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/plugin_register_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] updateMediaMeta content_id ===> '. print_r($data_arr, true) ."\r\n", FILE_APPEND);
