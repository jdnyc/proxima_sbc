...
function Ariel_ReceiveRequestInfo($aValue) {
	global $db;

	try{
		file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/Ariel_ReceiveRequestInfo'.date('Ymd').'.log', date('Y-m-d H:i:s')."\t".'aValue XML:::'.$aValue."\n", FILE_APPEND);
		
		$xml = simplexml_load_string($aValue);
		$v_data = (array) $xml;
		file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/Ariel_ReceiveRequestInfo'.date('Ymd').'.log', date('Y-m-d H:i:s')."\t".'aValue Array:::'.print_r($v_data, true)."\n", FILE_APPEND);

		$job_id = $v_data['jobId'];
		$progress = $v_data['progress'];
		$status = $v_data['jobStatus'];
		if($status == 'completed') $status = 'complete';
		$archive_request_info = $db->queryRow("SELECT * FROM BC_ARCHIVE_REQUEST WHERE IF_KEY1='".$job_id."'");
		$task_id = $archive_request_info['task_id'];
		$content_id = $archive_request_info['content_id'];
//$task_id, $cartridge_id, $content_id, $progress, $status
		$v_query = "select * from bc_task where task_id=".$task_id;

		@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/Ariel_ReceiveRequestInfo'.date('Ymd').'.log', date('Y-m-d H:i:s').'$v_query:::'.$v_query."\n", FILE_APPEND);

		$task = $db->queryRow($v_query);

		@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/Ariel_ReceiveRequestInfo'.date('Ymd').'.log', date('Y-m-d H:i:s').'$task:::'.print_r($task, true)."\n", FILE_APPEND);

		if(!empty($task)){
			$v_task_log_id = getSequence('TASK_LOG_SEQ');

			@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/Ariel_ReceiveRequestInfo'.date('Ymd').'.log', date('Y-m-d H:i:s').'$v_task_log_id:::'.$v_task_log_id."\n", FILE_APPEND);
			...
			if($status == 'processing'){
				...
				@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/Ariel_ReceiveRequestInfo'.date('Ymd').'.log', date('Y-m-d H:i:s').'task type:::'.$task['type']."\n", FILE_APPEND);
		...
		@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/Ariel_ReceiveRequestInfo'.date('Ymd').'.log', date('Y-m-d H:i:s').'$msg:::'.$msg."\n", FILE_APPEND);
