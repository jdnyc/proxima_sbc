...
function getPathPreview($request) {
	...
	try{
		...
		$usr_metas = $db->queryRow("
						SELECT	*
						FROM	".$usrmetatable."
						WHERE	USR_CONTENT_ID = $request
					");
		
		$sys_metas = $db->queryRow("
						SELECT	*
						FROM	".$sysmetatable."
						WHERE	SYS_CONTENT_ID = $request
					");
		...
		foreach($user_containers as $container) {
			...
			foreach($sub_usr_meta as $sub) {
				$usr_meta_field_code = 'usr_'.strtolower($sub['usr_meta_field_code']);
