	...
	try {
		...
		if($is_group == 'Y') {
			...
			if ($inserttype	== 0 ||  $inserttype == 1) {
                ...
                $filePathInfo = new \Api\Core\FilePath($filename);
                $filename = $filePathInfo->filenameExt;
				$server_ip = $requestxml['server_ip'];
				...
				if ($requestxml['metadata_type'] == 'id') {
                    ...
                    $metaValues		= MetaDataClass::getUserMetaValuesFromPost( $requestxml['metadata'] );
				...
				if (strstr($channel, 'ingest')) {
					$status =  \Api\Types\ContentStatusType::INGEST;
                }
               ...
               // $content_id = getSequence('SEQ_CONTENT_ID');
                ...
                $content = $contentService->createUsingArray($contentMeta, $contentStatus , [], $metaValues );
			...
			if ($inserttype	== 0 || $inserttype	== 2) {
                ...
                if ( ! empty($task_list_info)) {
                    ...
                    foreach ($task_list_info as $idx => $list_info) {
                        ...
                        //$task_list_info[$idx]['source'] = trim($task_list_info[$idx]['source'],'/');
                        //$task_list_info[$idx]['target'] = trim($task_list_info[$idx]['target'],'/');
                ...
                $container['searcher']->update($content_id);
			}
