...
function SoapUpdateTaskStatus($task_id, $cartridge_id, $content_id, $progress, $status) {
	@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/ODA_ODS_D_SoapUpdateTaskStatus'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] task_id ===> '.$task_id."\r\n", FILE_APPEND);
	global $server;
	global $db;
	$user_id = $_SESSION['user']['user_id'];

	@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/ODA_ODS_D_SoapUpdateTaskStatus'.date('Ymd').'.log', date('Y-m-d H:i:s').'SoapUpdateTaskStatus START:::'."\n", FILE_APPEND);

	try{
		$v_query = "select * from bc_task where task_id=".$task_id;

		@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/ODA_ODS_D_SoapUpdateTaskStatus'.date('Ymd').'.log', date('Y-m-d H:i:s').'$v_query:::'.$v_query."\n", FILE_APPEND);

		$task = $db->queryRow($v_query);

		@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/ODA_ODS_D_SoapUpdateTaskStatus'.date('Ymd').'.log', date('Y-m-d H:i:s').'$task:::'.print_r($task, true)."\n", FILE_APPEND);

		if(!empty($task)){
			$v_task_log_id = getSequence('TASK_LOG_SEQ');

			@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/ODA_ODS_D_SoapUpdateTaskStatus'.date('Ymd').'.log', date('Y-m-d H:i:s').'$v_task_log_id:::'.$v_task_log_id."\n", FILE_APPEND);
			...
			if($status == 'processing'){
				...
				@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/ODA_ODS_D_SoapUpdateTaskStatus'.date('Ymd').'.log', date('Y-m-d H:i:s').'task type:::'.$task['type']."\n", FILE_APPEND);
				if($task['type'] == 'restore' || $task['type'] == RESTORE){
					...
					$v_query = "
						...
						select	".$media_id.", content_id, storage_id, 'archive',
								path, filesize, '".date('YmdHis')."', 'ODS_D', 0
						from	bc_media
						where	content_id	= ".$content_id."
						and		media_type	= 'original'
					";
			...
			$v_query = "
				INSERT INTO BC_TASK_LOG
					(TASK_LOG_ID, TASK_ID, DESCRIPTION, CREATION_DATE, STATUS)
				values
					(".$v_task_log_id.", ".$task_id.", '".$v_msg."', '".date('YmdHis')."', '".$status."')
			";
		...
		@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/ODA_ODS_D_SoapUpdateTaskStatus'.date('Ymd').'.log', date('Y-m-d H:i:s').'$msg:::'.$msg."\n", FILE_APPEND);
