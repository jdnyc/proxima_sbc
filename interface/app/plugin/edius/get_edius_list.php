...
@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/'.basename(__FILE__).date('Ymd').'.log', date('Y-m-d H:i:s').$receive_xml."\n", FILE_APPEND);
...
try
{
	...
	switch($action)
	{
		case 'ud_content':
			...
			$ud_content->addAttribute('id', '4000282');
			$ud_content->addAttribute('name', '인제스트');
		...
		case 'category':
			...
			$category_info = $db->queryAll("select c.* from user_mapping u , bc_category c where c.category_id=u.category_id and  user_id='$user_id'  order by c.category_title ");
...
//			$category->addAttribute('hasChild', 'true');
		...
		case 'subprognm':
			...
			if(!empty($sub_info))
			{
				$prognm->addAttribute('startymd', trim($sub_info[count($sub_info)-1]['brodymd']));
				$prognm->addAttribute('endymd', trim($sub_info[0]['brodymd']));
			}
			...
			if(!empty($sub_info))
			{
				foreach($sub_info as $sub)
				{
					...
					$metaCtrl->addAttribute('prognm', trim($sub['prognm']));
					//$metaCtrl->addAttribute('subprogcd', $sub['subprogcd']);
					$subprogcdint = (int)$sub['subprogcd'];
					$subprogname = $subprogcdint.'회 ';
					$metaCtrl->addAttribute('subprognm', trim($subprogname.$sub['subprognm']));
					$metaCtrl->addAttribute('pd', trim($sub['korname']));
					$metaCtrl->addAttribute('brodymd', trim($sub['brodymd']) );
					$metaCtrl->addAttribute('medcd', trim($sub['medcd']) );
					$metaCtrl->addAttribute('progcd', trim($sub['progcd']) );
					$metaCtrl->addAttribute('formbaseymd', trim($sub['formbaseymd']) );
					$metaCtrl->addAttribute('subprogcd', trim($sub['subprogcd']) );
				}
			}
	...
	@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/'.basename(__FILE__).'error'.date('Ymd').'.log', date('Y-m-d H:i:s').$response->asXML()."\n", FILE_APPEND);
