...
$post_method = $_REQUEST['archive_method'];
	...
	*/
	...
	foreach($categories as $category) {
		...
		$is_category = $db->queryOne("
						SELECT	COUNT(*)
						FROM	BC_CATEGORY_ENV_ARCHIVE
						WHERE	CATEGORY_ID = $c_id
					");
		if($is_category == 0) {
			//없으면 신규 추가
			$db->exec( "
				INSERT INTO BC_CATEGORY_ENV_ARCHIVE
					(CATEGORY_ID, ARCHIVE_METHOD, ARCHIVE_S_TIME, ARCHIVE_E_TIME, EDIT_USER_ID, EDIT_DATETIME)
				values
			...
			//있으면 업데이트
			$db->exec("
				UPDATE	BC_CATEGORY_ENV_ARCHIVE 
				SET		ARCHIVE_METHOD = '$post_method',
						ARCHIVE_S_TIME = '$s_period',
						ARCHIVE_E_TIME = '$e_period',
