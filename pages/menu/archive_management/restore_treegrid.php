...
(function(){
	...
	function setup_win(){  
		var win = new Ext.Window({
			...
			buttons : [{
				...
				handler : function(){	
					var form = win.items.items[0].getForm();
					var restore_method = form.findField("restore_method").getValue();	
					...
					win.getComponent('setup_form').getForm().submit({
						url : '/pages/menu/archive_management/set_up_restore_config.php',
						params : {
							user_id : '<?=$user_id?>',
							mode : 'restore'
						},
	...
	var treegrid = new Ext.ux.tree.TreeGrid({
		...
		columns : [
			...
			{header : '요청승인 방법', dataIndex : 'restore_method', width : 90, align: 'center', hidden: true},
			{header : '요청승인 방법', dataIndex : 'restore_method_nm', width : 90, align: 'center'},
			{header : '요청승인 시작시간', dataIndex : 'restore_auth_start_time', width: 110, align: 'center',renderer: showEmpty},
			{header : '요청승인 종료시간', dataIndex : 'restore_auth_end_time', width: 110, align: 'center', renderer: showEmpty}
		],
		...
		tbar : [{
			...
			handler : function(){
				...
				if(sel) {
					...
					var restore_method = sel.attributes.restore_method;
					var acpt_period = sel.attributes.acpt_period;

					var restore_auth_start_time = sel.attributes.restore_auth_start_time;
					var restore_auth_end_time   = sel.attributes.restore_auth_end_time;
					...
					win_form.getComponent('c_method').setValue(restore_method);
					...
					if(restore_method == 'M') {
						...
						form.findField("s_period").setDisabled(false);
						form.findField("e_period").setDisabled(false);
						form.findField("s_period").setValue(restore_auth_start_time);
						form.findField("e_period").setValue(restore_auth_end_time);
					}
				} else {
		...
		dataUrl : '/pages/menu/archive_management/get_tree_grid_restore_data.php'
	});
