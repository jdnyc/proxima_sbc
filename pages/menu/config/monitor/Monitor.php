...
$allow_list = array(
	60,80,20,22,11,10,100,70,130,30,15,110,160,50,91,69
);
...
(function(){
		...
		Ariel.monitor.TaskPanel = Ext.extend(Ext.grid.GridPanel, {
			...
			initComponent: function(){
				...
				this.store = new Ext.data.JsonStore({
					...
					listeners: {
						beforeload: function(self, opts){
							...
							var searchType = Ext.getCmp('task_search_combo').getValue();
							self.baseParams = {
								taskType: g.taskType,
				...
				Ext.apply(this, {
					...
					columns: [
						columnRowIndex(50),
						{header: _text('MN01112'), dataIndex: 'user_task_name', width: 150},//'작업 흐름명'
						{header: 'ID', dataIndex: 'id', width: 60, hidden: true},
						{header: '작업유형명', dataIndex: 't_name', width: 120},//'모듈명'
						{header: _text('MN00236'), dataIndex: 'job_name', width: 200},//'모듈명'
						...
						{header: '콘텐츠유형', dataIndex: 'ud_content_title', width: 100},
						{header: _text('MN00171'), dataIndex: 'meta_media_id', width: 130, align:'center'},
						{header: _text('MN00249'), dataIndex: 'title', width: 250},
						{header: '재생길이', dataIndex: 'sys_video_rt', width: 80, align:'center'},
						...
						{header: '소스경로', dataIndex: 'source', width: 200},
						{header: '타겟경로', dataIndex: 'target', width: 200},
		...
		function buildTBarCanceled(that, av_mode){
			var button_text;
			if(av_mode == 'button'){
				button_text = '<span style="position:relative;top:1px;" title="'+'취소처리'+'"><i class="fa fa-minus-circle" style="font-size:13px;color:white;"></i></span>'; 
			}else{
				button_text = '<span style="position:relative;top:1px;"><i class="fa fa-minus-circle" style="font-size:13px;"></i></span>&nbsp;&nbsp;'+'취소처리';
			}
			return {
				//>>text: '취소처리',
				cls: 'proxima_button_customize',
		...
		return {
			...
			items: [{
				...
				runAutoReload : function(thisRef,count){
					this.stopAutoReload();
				...
				buildTbarDelete(this, 'button'),'->',
				{
					...
					handler: function(b, e){
						...
						if (b.pressed) {
							...
							TaskListTabPanel.runAutoReload(TaskListTabPanel,5000);
						} else {
                    ...
                    listeners:{
                        afterrender:function(self){
                            ...
                            function autoRefreshButtonMenuHandler(self){
                                ...
                                autoRefreshButton.setText(autoRefreshButtonText+'('+oriText+')');
								...
								if(count === false){
									...
									TaskListTabPanel.runAutoReload(TaskListTabPanel,count);
									autoRefreshButton.setIcon('/led-icons/accept.png');
								}
                            };
							autoRefreshButton.setText(autoRefreshButtonText+'(off)');
							autoRefreshButton.setIcon('/led-icons/cross.png');
