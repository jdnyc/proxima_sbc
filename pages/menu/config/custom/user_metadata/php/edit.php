...
function edit_table()
{
    ...
    // 우선 먼저 삭제 //
	$bf_info = $db->queryRow("select * from bc_ud_content where ud_content_code= '$ud_content_code'");

	if( empty($bf_info) || !( $bf_info['ud_content_id'] == $ud_content_id ) ){
		$editquery = MetaDataClass::alterTableQuery('usr' , $ud_content_id , $ud_content_code );
		$r = $db->exec($editquery);
	}
	...
	$query = "update bc_ud_content set
			  ...
			  ud_content_code='$ud_content_code',
...
function edit_field()
{
	...
	$usr_meta_field_code	= strtoupper(trim($_POST['usr_meta_field_code']));
	...
	{
		$beforeFieldInfo = $db->queryRow("select * from bc_usr_meta_field where ud_content_id=$ud_content_id and usr_meta_field_id = $usr_meta_field_id ");
		$before_usr_meta_field_code = strtoupper($beforeFieldInfo['usr_meta_field_code']);
		$is_exists = $db->queryOne("select count(*) from bc_usr_meta_field where ud_content_id=$ud_content_id and usr_meta_field_id != $usr_meta_field_id and usr_meta_field_code = '$usr_meta_field_code'");
		if($is_exists > 0) _print(_text('MSG01048'));//존재하는 테이블명입니다.

		if($before_usr_meta_field_code != $usr_meta_field_code){
			$query = MetaDataClass::alterFieldQuery('usr' , $ud_content_id , $before_usr_meta_field_code, $usr_meta_field_code );
			$r = $db->exec($query);
		}
	}

	if($summary_field_cd == '1') {
		$is_limit_thumbView = $db->queryOne("select count(*) from bc_usr_meta_field
			where ud_content_id=$ud_content_id and (summary_field_cd='1' or summary_field_cd='3')
				and usr_meta_field_id != '$usr_meta_field_id'");
		if($is_limit_thumbView > 0) _print('Thumbnail View metadata limit exceed(max: 1).');
	} else if ($summary_field_cd == '2') {
		$is_limit_summView = $db->queryOne("select count(*) from bc_usr_meta_field
			where ud_content_id=$ud_content_id and (summary_field_cd='2' or summary_field_cd='3')
				and usr_meta_field_id != '$usr_meta_field_id'");
	...
	$r = $db->exec("update bc_usr_meta_field set " .
						...
						"usr_meta_field_code	='$usr_meta_field_code', " .
						...
						"is_social		='$is_social', " .
						"num_line		='$number_of_line', " .
						"summary_field_cd	='$summary_field_cd' " .
						"where usr_meta_field_id=".$usr_meta_field_id." and ud_content_id=".$ud_content_id);
