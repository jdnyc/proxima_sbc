(function () {
	...
	var store = new Ext.data.JsonStore({
		//		autoLoad: true,
		url: '/store/statistics/content/download_ranking.php',
		root: 'down_rank',
		sortInfo: {
			field: 'rank',
			desc: 'desc'
	...
	return {
		...
		_text('MN00183')//!!'부터'
			...
			handler: function (btn, e) {
				Ext.getCmp('download_info').store.reload({
					params: {
						start: 0,
						limit: myPageSize,
			...
			handler: function (self, e) {
				...
				grid = Ext.getCmp('download_info');
				if (grid.store.totalLength == 0) {
					Ext.Msg.alert(_text('MN00023'), _text('MSG02051'));//'출력하실 내용이 없습니다.'
					return;
				}
				else {
					excelData('date', '/store/statistics/content/download_ranking.php', grid.colModel, '', search_value1, search_value2);
				}
			}
		}],
		...
		xtype: 'grid',
		id: 'download_info',
		title: '<span class="user_span"><span class="icon_title"><i class=""></i></span><span class="main_title_header">' + _text('MN00316') + '</span></span>',
		...
		cm: new Ext.grid.ColumnModel({
			...
			{header: '설명', dataIndex: 'description', width: 350},
			{header: '다운로드일자', dataIndex: 'date', renderer: Ext.util.Format.dateRenderer('Y-m-d'), width: 150}
			*/
				...
				//{header: _text('MN00049'), dataIndex: 'description', width: 350},
				{ header: _text('MN00101'), dataIndex: 'date', renderer: Ext.util.Format.dateRenderer('Y-m-d'), width: 150 }
