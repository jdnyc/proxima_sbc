...
(function(){
	...
	var store = new Ext.data.JsonStore({
		url: '/store/statistics/user/user_join.php',
		root: 'user_join',
		totalProperty: 'total',
		idProperty: 'user_id',
		sortInfo:{
	...
	return {
		...
		_text('MN00183'),
		{
			...
			handler: function(btn, e){
				Ext.getCmp('new_user_join').store.reload();
			}
		}],
		...
		cm: new Ext.grid.ColumnModel({
			...
			columns: [
				...
				//>>{header: '가입일자', dataIndex: 'date', renderer: Ext.util.Format.dateRenderer('Y-m-d'), width: 150},
				//>>{header: '최종 로그인일자', dataIndex: 'last_login', renderer: Ext.util.Format.dateRenderer('Y-m-d'), width: 150}
		...
		view: new Ext.ux.grid.BufferView({
			rowHeight: 18,
			scrollDelay: false,
			//>>emptyText: '지정기간 가입한 사용자가 없습니다.'
			emptyText: _text('MN00149')
		}),
