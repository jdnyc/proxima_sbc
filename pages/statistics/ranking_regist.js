(function () {
	...
	var store = new Ext.data.JsonStore({
		//		autoLoad: true,
		url: '/store/statistics/content/regist_ranking.php',
		root: 'regist_rank',
		sortInfo: {
			field: 'rank',
			desc: 'desc'
	...
	return {
		...
		_text('MN00183')//!!'부터'
			...
			handler: function (btn, e) {
				Ext.getCmp('regist_info').store.reload({
					params: {
						start: 0,
						limit: myPageSize,
			...
			handler: function (self, e) {
				...
				grid = Ext.getCmp('regist_info');
				if (grid.store.totalLength == 0) {
					Ext.Msg.alert(_text('MN00023'), _text('MSG02051'));//'출력하실 내용이 없습니다.'
					return;
				}
				else {
					excelData('date', '/store/statistics/content/regist_ranking.php', grid.colModel, '', search_value1, search_value2);
				}
			}
		}],

		xtype: 'grid',
		title: '<span class="user_span"><span class="icon_title"><i class=""></i></span><span class="main_title_header">' + _text('MN00317') + '</span></span>',
		...
		cm: new Ext.grid.ColumnModel({
			...
			columns: [
				...
				{header: '등록자', dataIndex: 'user', width: 150},
				{header: '등록일자', dataIndex: 'date', renderer: Ext.util.Format.dateRenderer('Y-m-d'), width: 150}
				*/
				{ header: _text('MN00204'), dataIndex: 'rank', width: 70 },
				{ header: _text('MN00291'), dataIndex: 'type', width: 120 },
				{ header: _text('MN00307'), dataIndex: 'title', width: 350 },
				{ header: _text('MN00120'), dataIndex: 'user', width: 150 },
				{ header: _text('MN00102'), dataIndex: 'date', renderer: Ext.util.Format.dateRenderer('Y-m-d'), width: 150 }
