(function () {
	...
	var store = new Ext.data.JsonStore({
		//	autoLoad: true,
		url: '/store/statistics/content/ranking_read.php',
		root: 'read_rank',
		sortInfo: {
			field: 'rank',
			desc: 'desc'
	...
	return {
		...
		_text('MN00183')//!!'부터'
			...
			handler: function (self, e) {
				...
				grid = Ext.getCmp('read_info');
				if (grid.store.totalLength == 0) {
					Ext.Msg.alert(_text('MN00023'), _text('MSG02051'));//'출력하실 내용이 없습니다.'
					return;
				}
				else {
					excelData('date', '/store/statistics/content/ranking_read.php', grid.colModel, '', search_value1, search_value2);
				}
			}
		}],
		...
		xtype: 'grid',
		id: 'read_info',
		title: '<span class="user_span"><span class="icon_title"><i class=""></i></span><span class="main_title_header">' + _text('MN00320') + '</span></span>',
		...
		cm: new Ext.grid.ColumnModel({
			...
			*/
				...
				{ header: _text('MN00253'), dataIndex: 'date', renderer: Ext.util.Format.dateRenderer('Y-m-d'), width: 150 }
