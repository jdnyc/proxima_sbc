	...
	*/
	...
	template: {
		...
		tile: new Ext.XTemplate(			
			'<div mode="thumb" class="{[this.getContentStatusCss(values.status)]}">',
				'<table class="x-grid3-row-table">',
					'<tbody>',
	...
	cm: new Ext.grid.ColumnModel({
		...
		columns: [
			...
			/*icons*/
			{
				header: _text('MN02325'), 
				dataIndex: 'icons_grid', 
				width: 90, 
				sortable: false,
				menuDisabled: true
			},
			...
			/*콘텐츠 상태*/
			{
				header: _text('MN02053'), 
				dataIndex: 'content_status_nm', 
                width:90,
                hidden: true, 
				menuDisabled: true,
				align: 'center',
			...
			if ( !in_array($ud_content_id, $CG_LIST ) ) {
				echo ",{header: '"._text('MN00102')."', dataIndex: 'created_date', width:120 , menuDisabled: true, renderer: Ext.util.Format.dateRenderer('Y-m-d'),renderer: function(value, metaData, record, rowIndex, colIndex, store) {metaData.css += 'column_content_data';return Ext.util.Format.date(value, 'Y-m-d');} }";
				echo ",{header: '"._text('MN00120')."', dataIndex: 'reg_user_nm', width:70, menuDisabled: true, renderer: function(value, metaData, record, rowIndex, colIndex, store) {metaData.css += 'column_content_data';return value;}}";
			}
