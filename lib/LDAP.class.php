...
//		$v_param['telnet_pwd'] = 'admin';
	...
	private function fn_ldap_modify_group_member($av_param){
		if($this->v_param['ldap_type'] == 'OD'){
			...
			$this->_log('fn_ldap_modify_group_member: OD');
			$this->_log('v_ldap_rdn: '.print_r($v_ldap_rdn, true) );
			$this->_log('v_info: '.print_r($v_info, true) );
	...
	function _log($log){
		if( isset($this->v_param['isLogFile']) &&  $this->v_param['isLogFile'] == true){
			file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/'.basename(__FILE__).date('Ymd').'.log', print_r($log, true)."\n", FILE_APPEND);
		}
		if( isset($this->v_param['isLogEcho']) &&  $this->v_param['isLogEcho'] == true ){
	...
	function _error($result, $function){
		if(!$result){
			...
			if(strstr( $error_msg , 'Already exists') || strstr( $error_msg , 'Type or value exists') ){
			}else{
	...
	function createSSHUser($user_info){
		...
		//계정생성
		$r = $ssh->cmdExec($defualt_command.$create_command ,//계정생성
			$defualt_command.$create_command." "."UserShell"." ".$UserShell	,//UserShell 입력
			$defualt_command.$create_command." "."RealName"." ".$user_id,//cn 입력
			$defualt_command.$create_command." "."UniqueID"." ".$user_number,//uidNumber 입력
			$defualt_command.$create_command." "."PrimaryGroupID"." ".$default_group_id,//gidNumber 입력
			$defualt_command.$create_command." "."NFSHomeDirectory"." ".$homeDirectory,//HomeDirectory 입력
			$defualt_command.$pwd_command //userpasswd 입력
		);
