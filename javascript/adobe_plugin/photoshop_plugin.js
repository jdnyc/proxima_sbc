  ...
  //  var seq_id   = sel.get('seq_id');
    ...
    if(ori_path){
        ...
        if(children){
          ...
          window.top.load_document_fn(content_id, deferreds);
          return ;
...
function _photoshop_put_image()
{
  top.save_document_before_check_fn();
  //register_sequence_form();
}

/* 등록 시 데이터 저장할 변수 */
var image_regist_dat = "";

/* 등록 시 팝업 창 클로벌 변수 */
var register_image_form_win_global = "";
...
/* 등록시 팝업창 */
function register_image_form(path,title){

	// 이미 생성시 return
	if(register_image_form_win_global){
		return;
	}

	var register_image_form_win = new Ext.Window({
			title: _text('MN02511'), //포토샵 플러그인 - 등록
			width: 500,
			modal: true,
			height: 350,
			layout: 'border',
			...
			listeners:{
				...
				close: function(self) {
					register_image_form_win_global = "";
				}
			},
			buttons: [{
					...
					handler: function(b, e){
						register_image_form_win.close();
						register_image_form_win_global = "";
					}
				},{
					text: '<span style="postion:relative;top:1px;font-family:\'나눔고딕\' !important;"><i class="fa fa-sign-in" aria-hidden="true">&nbsp;'+_text('MN02512')+'</i></span>',//시퀀스로 등록
					scale: 'medium',
					handler: function(b, e){
						if(register_image_form_win.get(0).get(0).get(0).getForm().isValid())
						{
								register_image_form_win.el.mask();
								var metadata = [];
								...
								//CONTENT_ID 를 가져 써야함!!!
								image_regist_dat = ret;
								// Export 함수 호출
								get_image_info('image_save');
						}
					}
				}
			],
				...
				tbar:[{
			          ...
			          editable: false,
								...
								store: new Ext.data.JsonStore({
									...
									baseParams: {
										kind : 'ud_content',
                    bs_content_id : Ext.isPhotoshopBsContent_id,
										flag: ''
									},
									fields: [
										'ud_content_title',
							...
							callback: function(opts, success, response){
								if (success) {
									try {
										...
										if(self.isFirst){
											self.isFirst = false;
											if(register_image_form_win.getHeight() < self.getHeight()){
												var setHeight_v = self.getHeight() + default_set_margin_height;
												register_image_form_win.setHeight(setHeight_v);
											}
										}
										else {
											var setHeight_v = self.getHeight() + default_set_margin_height;
	...
	register_image_form_win.show();
  register_image_form_win.hide();
	register_image_form_win_global = register_image_form_win;
}

/* 실제 정보로 등록하는 Ajax */
function regist_photoshop_image_(type,path,seq_id,content_id)
{
  //alert('regist_photoshop_image_');
	var render_exec_url = '/plugin/photoshop/put_photoshop.php';

	if(type == 'fail')
	{
		register_image_form_win_global.el.unmask();
	...
	else if(type == 'jpeg')
	{
        ...
        content_id : content_id
			...
			register_image_form_win_global.el.unmask();
				...
				if(res.success) {
					register_image_form_win_global.close();
	...
	Ext.Ajax.request({
		...
		callback: function(self, success, response){
			if (success) {
                ...
                //alert('top_svae Call');
								top.save_document_fn(save_path,save_content_id);
					}
				} catch (e) {
