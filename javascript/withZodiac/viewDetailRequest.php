...
$graphic_reqest_ty = $_REQUEST['graphic_reqest_ty'];
...
(function(records){
                    ...
                    store: new Ext.data.JsonStore({
                        ...
                        listeners: {
                            load: function (store, r) {
								if('<?=$ord_meta_cd?>' === 'Graphic'){
									form_editor.getForm().findField('graphic_reqest_ty').setValue('<?=$graphic_reqest_ty?>');
								}
	...
	var form_news = new Ext.FormPanel({
		...
		items:[{
				...
				items:[{
					...
					xtype : 'textarea',
					style: 'font-size: 15px; line-height: 1.5em;',
	...
	var grid_edl = new Ext.grid.GridPanel({
		...
		cm: new Ext.grid.ColumnModel({
			...
			columns: [
				new Ext.grid.RowNumberer(),
				{header: _text('MN00249'), dataIndex: 'edl_titl', width: 30},//'제목'
				{header: 'Script ID', dataIndex: 'ord_id', width: 60, hidden: true},
				{header: _text('MN02105'), dataIndex: 'mark_in', width: 20,},//'시작'
	...
	function clickbtn(selected_row){
		Ext.each(selected_row, function(r){
            //var aTag  = document.getElementById('downloadImg');

			//if(aTag){
			//document.body.removeChild(aTag);
			//} 
            var url = '/custom/ktv-nps/download/download.php' + '?type=attach&fileid=' + r.json.id;
             window.open(url);          
			...
			handler: function(btn, e){
				if(btn.ownerCt.ownerCt.selModel.getSelections().length < 1 ){
					...
					Ext.Msg.show({
						...
						fn : function(btn){
							if(btn == 'ok'){
								clickbtn(grid_file.getSelectionModel().getSelections());
							}
						}
					});
				}
...
function get_request_count_info(){
	...
	var win = new Ext.Window({
		...
		listeners: {
			close: function(){
				Ext.getCmp('request_list').getStore().reload();
				<!-- get_request_count_info();의뢰요청개수 정보 --> 
			}
		},
		buttons : [{
