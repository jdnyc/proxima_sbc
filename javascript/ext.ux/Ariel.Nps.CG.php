...
Ariel.Nps.CG = Ext.extend(Ext.Panel, {
            layout: 'border',
			...
			plain: true,
                        ...
                        _simpleSearch: function(){
                            var value = Ext.getCmp('simple_search_cg').getValue();
                            if(!value){
                            ...
                            var p = Ext.getCmp('tab_warp_cg').get(0);
                            i.store.load({
                                params: {
                                    task: 'simple_search',
                                    value: value
                        ...
                        items:[{
                            ...
                            listeners: {
                                render: function(self){
                                    var init_path = Ext.getCmp('menu-tree-cg').getRootNode().getPath();
                                    Ext.getCmp('menu-tree-cg').selectPath( init_path );
                                },
                                beforetabchange: function(self, n, c){
                                    if( !Ext.isEmpty(Ext.getCmp('nav_tab_cg')) ){
                                        var tree, treeLoader, rootNode;
                                        tree = Ext.getCmp('menu-tree-cg');
                                        ...
                                        var beforeSelNode = Ext.getCmp('menu-tree-cg').getSelectionModel().getSelectedNode();
                                        ...
                                        Ext.Ajax.request({
                                                ...
                                                callback: function(opt, success, response){
                                                        ...
                                                        try{
                                                                ...
                                                                var value = Ext.get('search_input_cg').dom.value;
                                                                ...
                                                                var category = Ext.getCmp('menu-tree-cg').getSelectionModel().getSelectedNode();
                                                                if (!Ext.getCmp('menu-tree-cg').getRootNode().attributes.read)
                                                                {
                                                                        Ext.getCmp('menu-tree-cg').getRootNode().expand( false, true, function(n){

                                                                                var find = false;

                                                                                var find = n.findChild('read', 1);

                                                                                if(Ext.isEmpty(Ext.getCmp('menu-tree-cg').getLoader().baseParams.beforePath))
                                                                                {
                                                                                        if(!Ext.isEmpty(find))
                                                                                        {
                                                                                                find.select();
                                                                                                ...
                                                                                                Ext.getCmp('menu-tree-cg').getLoader().baseParams.beforePath = args.filter_value;
                                                                                                r.reload(args);
                                                                                        }
                                                                                }
                                                                                else
                                                                                {
                                                                                                var args = {};
                                                                                                args.filter_type =	'category';
                                                                                                args.filter_value =	Ext.getCmp('menu-tree-cg').getLoader().baseParams.beforePath;

                                                                                                Ext.getCmp('menu-tree-cg').selectPath(args.filter_value);
                                                                                                Ext.getCmp('menu-tree-cg').getLoader().baseParams.beforePath = args.filter_value;
                                                                                                r.reload(args);
                                                                                }
                                                                ...
                                                                if(Ext.isAir)
                                                                {
                                                                        Ext.getCmp('tab_warp_cg').doReload.defer(2000);
                                                                }
					...
					if (!Ext.getCmp('menu-tree-cg').getRootNode().attributes.read) {
							...
							if(Ext.isEmpty(Ext.getCmp('menu-tree-cg').getLoader().baseParams.beforePath)) {
                                        ...
                                        if( !Ext.isEmpty(Ext.getCmp('menu-tree-cg')) ){
                                ...
                                items: [
                                    <?php
                                        ...
                                        foreach ($tabs as $tab){
                                            if( !in_array(  $tab['ud_content_id'], $CG_LIST ) ){
                                                continue;
                                            }
			    ...
			    render: function(self){
                        ...
                        listeners: {
                            check: function(self, checked) {
                                if(checked) {
                                    Ext.getCmp('search_input_cg').setValue('');
                                }
                            }
                        }
                }]
        ...
        Ariel.Nps.CG.superclass.initComponent.call(this);
