							...
							[
								{
									...
									id : 'preview_content',
									height: Ext.getBody().getViewSize().height*0.9/2-50,
									xtype : 'panel'
								},{
									region: 'center',
									border: false,
									height: 28,
									html: '<audio id="player2" class="" preload="auto" controls style="width:100%;"><source id="player2_source" src="" type="audio/mpeg"></audio>',
									xtype : 'panel'
											...
											sm: new Ext.grid.RowSelectionModel({
												...
												listeners: {
													...
													rowselect: function(selModel, rowIndex, e) {
														...
														source.src = path_proxy;
														var playerObj = document.getElementById('player2');
														playerObj.load();
														//playerObj.play();
														
														var selections = Ext.getCmp('tab_warp').getActiveTab().get(0).getSelectionModel().getSelections();
														Ext.each(selections, function(selection){
															if(selection.id == content_id){
																var data = selection.json;
																var path_preview = data.lowres_web_root +'/'+data.thumb;
