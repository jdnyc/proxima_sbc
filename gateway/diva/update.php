...
// <Request><archive_list><archive object_name="20130703V77513" reqnum="13900" task_id="1063" type="restore" status="complete" progress="100" filesize="0" error_id="1000" error_msg="(success)"/></archive_list></Request>
// 10.10.50.127	[2019-10-28 16:42:22] status ===> comp
try {	
	...
	$expired_date = date('YmdHis', strtotime("+14 days") );

	$archive_list = new SimpleXMLElement(file_get_contents('php://input'));
	
	foreach ($archive_list->archive_list->archive as $item) {
		//print_r($item);exit;
		@file_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/diva_update_'.date('Ymd').'.log', $_SERVER['REMOTE_ADDR']."\t[".date('Y-m-d H:i:s').'] ===> '.print_r($item ,true)."\n", FILE_APPEND);
		//기본 flag 
        ...
        ");
        $rootTaskId = $task_info['root_task'];

		if($status != 'error') {
			$result = $db->exec("
                UPDATE	ARCHIVE
                SET		REQNUM = '{$reqnum}'
                WHERE	(TASK_ID = '$task_id' or TASK_ID='$rootTaskId' )
            ");
...
// 				");
		...
		$result = $task->Post_XML_Soket('127.0.0.1', '/workflow/update_task_status.php', $sendxml );
		...
		if($status == 'complete') {
			...
			if ($type == 'archive') {
                ...
                $archiveInfo = $db->queryRow("
                SELECT	*
                    FROM	archive
                    WHERE	(TASK_ID = '$task_id' or TASK_ID='$rootTaskId' )
                ");

                $archiveMedia = new \Api\Models\ArchiveMedia();
                $archiveMedia->content_id   = $task_info['src_content_id'];
                $archiveMedia->media_id     = $archiveInfo['media_id'];
                $archiveMedia->object_name  = $archiveInfo['archive_id'];
                $archiveMedia->archive_category     =  $archiveInfo['diva_category'];
                $archiveMedia->archive_group        = $archiveInfo['diva_group'];
                $archiveMedia->user_id = $arc_user_id;
                $archiveMedia->save();
				...
				// 		$data = $engine->update($radio_restore_content_id, 'proxima_das');
				// 	}	
				// }
                ...
                $media = \Api\Models\Media::where('media_id',$media_id)->first();
                if( !empty($media) ){
                    if(!empty($filesize)){
                    ...
                    $media->status = 0;
                    $media->flag = null;
                    $media->delete_status = null;
                    $media->delete_date = null;
                    $media->path = $task_info['path'];
                    $media->save();
                }
                ...
                $archiveMedia->where('content_id',$task_info['src_content_id'] )->where('object_name', $object_name)->delete();
