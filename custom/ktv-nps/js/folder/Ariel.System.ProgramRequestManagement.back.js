(function () {
  ...
  Ariel.System.ProgramRequestManagement = Ext.extend(Ext.Panel, {
    ...
    _initialize: function () {
      ...
      var cm = new Ext.grid.ColumnModel({
        ...
        columns: [
          ...
          { header: '프로그램 아이디', dataIndex: 'pgm_id', width: 150 },
          { header: '사유', dataIndex: 'dc', width: 300 },
          {
            header: '상태', dataIndex: 'status', align: 'center', renderer: function (value) {
              if (value == null) {
                return null;
              } else {
                switch (value) {
          ...
          { header: '신청자', dataIndex: 'regist_user_id' },
          { header: '승인자', dataIndex: 'approval_user_id' },
      ...
      this.listeners = {
        afterrender: function (self) {
          _this._initializeByPermission = function (permissions) {
            var _this = this;
            // var grid = Ext.get(this.gridId);
            // var win = Ext.get(this.windowId);
            var tbarIdx = 0;
            ...
            if (true) {
              grid.getTopToolbar().insertButton(tbarIdx, {
                xtype: 'aw-button',
                iCls: 'fa fa-refresh',
                text: '새로고침',
            ...
            if (true) {
              grid.getTopToolbar().insertButton(tbarIdx, _this.startDateField);
              tbarIdx++;
            };
            if (true) {
              grid.getTopToolbar().insertButton(tbarIdx, {
            ...
            if (true) {
              grid.getTopToolbar().insertButton(tbarIdx, _this.endDateField);
              tbarIdx++;
            };
            if (true) {
              grid.getTopToolbar().insertButton(tbarIdx, {
    ...
    _updateStatusAjax: function (updateStatus) {
      var _this = this;
      var sel = Ext.getCmp(this.gridId).getSelectionModel().getSelected();
